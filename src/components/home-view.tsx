    ...
    <>
      ...
      {featured && (
        <section className="mt-6 rise">
          {/* Carte média : dégradé saturé + voile noir, donc texte blanc. */}
          <div className="relative overflow-hidden rounded-[32px] text-white">
            ...
            <div className="absolute left-5 right-5 top-5 flex items-start justify-between">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-black/35 px-3 py-1.5 text-[11px] font-medium leading-none backdrop-blur-md">
                <span className="h-1.5 w-1.5 rounded-full bg-gold-400" />
                À la une
              </span>
              <span className="rounded-full bg-black/35 px-3 py-1.5 text-[11px] font-medium leading-none backdrop-blur-md">
                {compact(featured.monthlyListeners)} / mois
              </span>
            </div>
            ...
            <div className="absolute inset-x-5 bottom-5">
              ...
              <div className="mt-4 flex gap-2.5">
                ...
                <Link
                  ...
                  className="flex h-13 flex-1 items-center justify-center gap-2 rounded-full bg-white/15 text-[15px] font-semibold backdrop-blur-md active:scale-[.98]"
                >
                  Voir la page
          ...
          {ranking.map(({ artist, total, count }, i) => (
            <Link key={artist.id} href={`/a/${artist.slug}`} className="block">
              <Glass className="flex items-center gap-3 px-3 py-3 transition active:scale-[.99]">
                ...
                <Avatar name={artist.name} gradient={artist.gradient} size={46} />
